...
# Import tree validation functions
from refactor.cli.formatting.tree.validation import validate_tree_structure, fix_tree_structure

def make_template_optional(parser, help_text="Path to the task file"):
    """
    Make the template argument optional in a command parser.
    
    This utility function modifies a parser to make the template argument optional,
    allowing it to work with the default template mechanism.
    
    Args:
        parser: ArgumentParser to modify
        help_text: Help text for the template argument
        
    Returns:
        Modified parser with optional template argument
    """
    # Create a more helpful message that indicates default template support
    full_help_text = f"{help_text} (defaults to .project/clickup_tasks.json if it exists)"
    
    # Add the template argument with nargs='?' to make it optional
    parser.add_argument('template', nargs='?', help=full_help_text)
...
def format_hierarchical_tree(
    ...
    for i, task in enumerate(current_level_tasks):
        ...
        # Get task type emoji if available in the task
        task_type = task.get("task_type", "task")
        from refactor.cli.formatting.common.task_info import TASK_TYPE_EMOJI_MAP
        type_emoji = TASK_TYPE_EMOJI_MAP.get(task_type, "📝")
        task_details += f"{type_emoji} "
        
        # Add task ID in square brackets if requested
        if show_ids and (task_id := task.get("id")):
            if colorize_output:
                # Import id_prefix_color function
                from refactor.utils.colors import id_prefix_color, colorize
                colored_id = colorize(f"[{task_id}]", id_prefix_color(task_id))
                task_details += f"{colored_id} "
            else:
        ...
        if tags := task.get("tags", []):
            ...
            tag_prefix = prefix + ("  " if is_last else "│ ")
...
def _format_task_info(
    ...
    Args:
        ...
        show_score: Whether to show task score
    ...
    if colorize_output:
        from refactor.utils.colors import (
            TextColor, colorize, status_color, priority_color, DefaultTheme
        )
    ...
    if show_full:
        # Include ID for full display
        if colorize_output:
            output = f"{task_name} ({colorize(task_id, TextColor.BRIGHT_BLACK)})"
        else:
            output = f"{task_name} ({task_id})"
    else:
        output = task_name
    
    # Add status with color if enabled
    if colorize_output:
        status_text = f" [{colorize(task_status, status_color(task_status))}]"
    else:
        status_text = f" [{task_status}]"
    output += status_text
    
    # Add priority with color if enabled
    if colorize_output:
        priority_text = f" (P{colorize(task_priority, priority_color(int(task_priority)))})"
    else:
        priority_text = f" (P{task_priority})"
    output += priority_text
    
    # Add task type emoji if requested
    if show_type_emoji:
        from refactor.cli.formatting.common.task_info import TASK_TYPE_EMOJI_MAP
        type_emoji = TASK_TYPE_EMOJI_MAP.get(task_type, "📝")
        output = f"{type_emoji} {output}"
    
    # Add tags if requested
    if show_tags and task_tags:
        tags_text = " ["
        for i, tag in enumerate(task_tags):
            if i > 0:
                tags_text += ", "
            
            if colorize_output:
                tags_text += colorize(tag, DefaultTheme.TAG)
            else:
                tags_text += tag
    ...
    # Add completion score if requested
    if show_score:
        from refactor.cli.formatting.common.task_info import format_task_score
        score_text = format_task_score(task, colorize_output)
        if score_text:
            output += f" {score_text}"
...
def format_task_hierarchy(
    ...
    show_type_emoji: bool = True,
    validate_pipes: bool = True,
    # Add new parameters for content display
    ...
    """
    Format tasks hierarchically with different organization options.
    ...
    Args:
        ...
        show_full: Whether to show full task details
        organize_by_container: Whether to organize by container
        dependency_as_tree: Whether to organize by dependency
        dependency_type: Type of dependency relationship to use
        include_completed: Whether to include completed tasks
        hide_orphaned: Whether to hide orphaned tasks
        show_score: Whether to show task scores
        show_tags: Whether to show tags for each task
        tag_style: Style for tag display
        show_type_emoji: Whether to show task type emoji
        ...
        show_comments: Number of recent comments to show (0 for none)
    
    Returns:
        Formatted string with hierarchical tasks
    """
    # If no tasks, return empty result
    if not tasks:
        return "No tasks found."
    
    # Reference to TAG_EMOJI
    from refactor.cli.formatting.tree.validation import TAG_EMOJI
    
    # If detailed view requested, override content display settings
    if show_details:
        show_descriptions = True
        show_relationships = True
        show_comments = 3  # Show some recent comments
    
    lines = []
    
    # Set of task IDs that have been displayed
    displayed_tasks = set()
    
    # Set of task IDs that are orphaned
    orphaned_task_ids = set()
    
    # Build a mapping of task IDs to tasks
    tasks_map = {task.get('id'): task for task in tasks if task.get('id')}
    
    # Handle organization by container
    ...
    if dependency_as_tree:
        # Default dependency type if not specified
        ...
        # Initialize dependency graph for all tasks
        for task in tasks:
            task_id = task.get('id')
            if task_id:
                dependency_graph[task_id] = set()
                if task_id not in reverse_dependency_graph:
                    reverse_dependency_graph[task_id] = set()
        
        # Fill dependency graph based on relationships
        for task in tasks:
            task_id = task.get('id')
            relationships = task.get('relationships', {})
            
            # Get dependencies of the specified type
            dependencies = relationships.get(dependency_type, [])
            ...
            for dep_id in dependencies:
                if dep_id in tasks_map:
                    ...
                    # Also update reverse graph
                    if dep_id not in reverse_dependency_graph:
                        reverse_dependency_graph[dep_id] = set()
                    reverse_dependency_graph[dep_id].add(task_id)
        
        # Find root tasks (those that are depended on but don't depend on others)
        root_task_ids = set()
        
        # Tasks that are depended on but don't depend on others
        for task_id in reverse_dependency_graph:
            if task_id not in dependency_graph or not dependency_graph[task_id]:
                root_task_ids.add(task_id)
        
        # For any task not in the dependency graph, check if others depend on it
        for task in tasks:
            task_id = task.get('id')
            if task_id and task_id in reverse_dependency_graph and (
                task_id not in dependency_graph or not dependency_graph[task_id]
            ):
                root_task_ids.add(task_id)
        
        # If no root tasks found, fall back to tasks with no dependencies
        if not root_task_ids:
            for task_id in dependency_graph:
                if not dependency_graph[task_id]:
                    root_task_ids.add(task_id)
        
        # If still no root tasks, include all tasks as roots
        if not root_task_ids:
            root_task_ids = {task.get('id') for task in tasks if task.get('id')}
        
        # Convert root task IDs to task objects and sort by priority
        root_tasks = [tasks_map[task_id] for task_id in root_task_ids if task_id in tasks_map]
        ...
        # Function to recursively format a task and its dependents
        def format_dependency_tree(task_id, level=0, branch_map=None, is_last=False):
            if not branch_map:
                branch_map = {}
            ...
            # Build prefix based on branch map
            prefix = ""
            for l in range(level):
                if l in branch_map and branch_map[l]:
                    prefix += "│ "
            ...
            if str(task.get('status', '')).lower() == 'complete' and not include_completed:
                return result
            
            # Format task info
            task_line = _format_task_info(task, colorize_output, show_full, show_score, show_tags, tag_style, show_type_emoji)
            result.append(f"{prefix}{connector}{task_line}")
            
            # Content display prefix - ensure it aligns with tree structure
            # CRITICAL: This must maintain pipe alignment for tree validation
            content_prefix = prefix + ("  " if is_last else "│ ")
            
            # Add description if requested
            if show_descriptions and task.get('description'):
                description = task.get('description', '')
                # Process description containing newlines
                if '\n' in description:
                    # Split description into lines and process each
                    desc_lines = description.split('\n')
                    # Add description header with icon
                    result.append(f"{content_prefix}📝 Description:")
                    
                    # Process each line with proper indentation and pipe alignment
                    desc_content_prefix = content_prefix + "  "  # Additional indentation
                    for line in desc_lines:
                        if line.strip():  # Skip empty lines
                            if len(line) > description_length:
                                line = line[:description_length-3] + '...'
                            result.append(f"{desc_content_prefix}{line}")
                else:
                    # Single line description
                    if len(description) > description_length:
                        description = description[:description_length-3] + '...'
                    result.append(f"{content_prefix}📝 {description}")
            
            # Add relationships if requested
            if show_relationships and task.get('relationships'):
                relationships = task.get('relationships', {})
                for rel_type, related_tasks in relationships.items():
                    if not related_tasks:
                        continue
                    
                    # Format relationship type title
                    rel_title = rel_type.replace('_', ' ').title()
                    
                    # Add relationship header with consistent pipe alignment
                    result.append(f"{content_prefix}🔄 {rel_title}:")
                    
                    # Add each related task (up to 3)
                    for i, rel_task_id in enumerate(related_tasks[:3]):
                        # Try to get related task name
                        rel_task_name = "Unknown Task"
                        if rel_task_id in tasks_map:
                            rel_task = tasks_map[rel_task_id]
                            rel_task_name = rel_task.get('name', 'Unknown Task')
                        
                        # Add related task with proper indentation and consistent pipe alignment
                        result.append(f"{content_prefix}  ∟ {rel_task_name}")
                    
                    # If there are more than 3 related tasks, add a count
                    if len(related_tasks) > 3:
                        result.append(f"{content_prefix}  ∟ ... and {len(related_tasks) - 3} more")
            ...
            if show_comments > 0 and task.get('comments'):
                ...
                # Skip if no comments
                if comments:
                    # Calculate how many comments to show (up to limit)
                    comment_count = min(show_comments, len(comments))
                    
                    # Get the most recent comments
                    recent_comments = comments[-comment_count:]
                    
                    # Get indentation prefix for comments
                    # CRITICAL: Maintain consistent pipe alignment for tree validation
                    comment_prefix = prefix + ("  " if is_last else "│ ")
                    
                    # Add comment header with speech bubble emoji
                    result.append(f"{comment_prefix}💬 Recent Comments:")
                    
                    # Process each comment
                    for comment in recent_comments:
                        comment_text = comment.get('text', '')
                        comment_author = comment.get('author', 'Anonymous')
                        comment_date = comment.get('date', 'Unknown date')
                        
                        # Convert escaped newline sequences to actual newlines
                        comment_text = _convert_escaped_newlines(comment_text)
                        
                        # Add comment info with proper indentation
                        # CRITICAL: Use additional level of indentation while maintaining pipe alignment
                        sub_prefix = comment_prefix + "  "
                        lines.append(f"{sub_prefix}From: {comment_author} on {comment_date}")
                        
                        # Process comment text, handle multi-line comments properly
                        if '\n' in comment_text:
                            ...
                            for i, line in enumerate(comment_lines):
                                if line.strip():  # Skip empty lines
                                    if len(line) > description_length:
                                        line = line[:description_length-3] + '...'
                                    lines.append(f"{sub_prefix}{line}")
                                else:
                                    # Preserve empty lines with proper indentation
                                    lines.append(f"{sub_prefix}")
                        else:
                            # Single line comment
                            if len(comment_text) > description_length:
                                comment_text = comment_text[:description_length-3] + '...'
                            lines.append(f"{sub_prefix}{comment_text}")
                        
                        # Add separator between comments with proper pipe alignment
                        lines.append(f"{sub_prefix}─────") # Separator between comments
            
            # Get dependent tasks
            dependent_ids = list(reverse_dependency_graph.get(task_id, set()))
            dependent_tasks = []
            
            # Convert IDs to tasks and sort
            for dep_id in dependent_ids:
                if dep_id in tasks_map:
                    dependent_tasks.append(tasks_map[dep_id])
            
            # Sort dependents by priority
            dependent_tasks.sort(key=lambda t: (
                str(t.get('status', '')).lower() == 'complete',  # Completed tasks at end
                int(t.get('priority', 3))       # Higher priority first
            ))
            
            # Process each dependent
            for i, dep_task in enumerate(dependent_tasks):
                is_last_dep = i == len(dependent_tasks) - 1
                
                # Update branch map for the next level
                new_branch_map = branch_map.copy()
                new_branch_map[level] = not is_last_dep
                
                # Format the dependent and its dependents
        ...
        # Format each root task with its dependents
        dependency_lines = []
        for i, root_task in enumerate(root_tasks):
            is_last = i == len(root_tasks) - 1
            branch_map = {0: not is_last}
            
            # Format the root task and its dependents
            task_lines = format_dependency_tree(
                root_task.get('id'),
                level=0,
            ...
            dependency_lines.extend(task_lines)
        
        # Add relationship type information
        dependency_header = "Tasks organized by dependency relationships"
        if colorize_output:
            from refactor.utils.colors import TextColor, colorize
            dependency_header = colorize(dependency_header, TextColor.BRIGHT_BLUE)
        
        lines.append(dependency_header)
        lines.append(f"Relationship type: {dependency_type}")
        lines.append("")  # Empty line before the tree
        
        # Add dependency tree to output
    ...
    # Handle regular parent-child hierarchy
        ...
        for i, task in enumerate(sorted_root_tasks):
            is_last = i == len(sorted_root_tasks) - 1
            ...
            branch_map = {0: not is_last}
            task_lines = _format_hierarchical_task(
                task,
                tasks_map,
                level=0,
                indent=indent,
                colorize_output=colorize_output,
                show_relationships=show_relationships,
                core_manager=None,  # We don't have access to core_manager here
                show_content_preview=show_descriptions,
                content_preview_length=description_length,
                show_comments=show_comments,
            ...
            if not hide_orphaned and orphaned_task_ids and i == len(sorted_root_tasks) - 1:
                ...
                if orphaned_tasks:
                    ...
                    for p_idx, (parent_id, o_tasks) in enumerate(orphan_parents.items()):
                        ...
                        for o_idx, o_task in enumerate(o_tasks):
                            ...
                            # Skip completed tasks unless include_completed is True
                            if str(o_task.get('status', '')).lower() == 'complete' and not include_completed:
                                continue
    ...
    if validate_pipes:
        ...
        if not valid:
            ...
            for error in errors:
                logging.warning(error)
            lines = fix_tree_structure(lines)
...
def _format_container_hierarchy(
    ...
    show_type_emoji: bool = True,
    validate_pipes: bool = True,
    # Add content display options parameters
    show_descriptions: bool = False,
    description_length: int = 100,
    show_relationships: bool = False,
    show_comments: int = 0
    ...
    # Import color utilities if needed
    if colorize_output:
        from refactor.utils.colors import TextColor, colorize, container_color, completion_stats_color
    ...
    def format_container(name, container_type, task_count, completed_count, id_str=""):
        ...
        if colorize_output:
            ...
            return f"{colorize(name, color)} {colorize(stats, stats_color)}{' ' + colorize(id_str, TextColor.BRIGHT_BLACK) if id_str and show_ids else ''}"
        else:
            return f"{name} {stats}{' ' + id_str if id_str and show_ids else ''}"
    ...
    for space_idx, (space_id, space) in enumerate(containers['spaces'].items()):
        ...
        for folder_idx, folder_id in enumerate(space_folders):
            ...
            if is_last_space and (hide_orphaned or not top_level_orphaned_tasks):
                ...
                folder_prefix = "│ "  # Space continues, pipe needed
            ...
            for list_idx, list_id in enumerate(sorted(folder_lists)):
                ...
                if is_last_space and (hide_orphaned or not top_level_orphaned_tasks):
                    ...
                    list_prefix += "│ "  # Space continues, need pipe
                    
                if is_last_folder:
                    list_prefix += "  "  # Folder is last, no pipe
                else:
                    list_prefix += "│ "  # Folder continues, need pipe
                ...
                if is_last_space and (hide_orphaned or not top_level_orphaned_tasks):
                    ...
                    task_prefix += "│ "  # Space continues, need pipe
                    
                if is_last_folder:
                    task_prefix += "  "  # Folder is last, no pipe
                else:
                    task_prefix += "│ "  # Folder continues, need pipe
                    
                if is_last_list:
                    task_prefix += "  "  # List is last, no pipe
                else:
                    task_prefix += "│ "  # List continues, need pipe
    ...
    if top_level_orphaned_tasks and not hide_orphaned:
        ...
        for task in top_level_orphaned_tasks:
            ...
            if not include_completed and str(task.get('status', '')).lower() == 'complete':
                continue
        ...
        sorted_orphans = sorted(
            sorted_orphans,
            key=lambda t: (
                t.get('status') == 'complete',  # Incomplete first
                int(t.get('priority', 4)),                 # Lower priority first
                t.get('name', '')                          # Alphabetical
            )
        )
    ...
    if hide_orphaned:
        ...
        completed_tasks = sum(1 for task in tasks if str(task.get('status', '')).lower() == 'complete' 
    ...
    if not include_completed:
        total_tasks = sum(1 for task in tasks if str(task.get('status', '')).lower() != 'complete' 
...
def _format_container_tasks(
    ...
    for root_idx, root_task in enumerate(sorted_root_tasks):
        ...
        task_description = _format_task_basic_info(root_task, colorize_output, show_ids, show_score, show_tags, tag_style, show_type_emoji)
...
def _format_orphaned_list_tasks(
    ...
    if not hide_orphaned:
        ...
        if orphaned_tasks:
            ...
            # Sort orphaned tasks
            orphaned_tasks.sort(key=lambda t: (
                str(t.get('status', '')).lower() == 'complete',  # Incomplete first
                int(t.get('priority', 4))
            ))
            ...
            # Format each orphaned task
            for orphan_idx, orphan in enumerate(orphaned_tasks):
                is_last_orphan = orphan_idx == len(orphaned_tasks) - 1
                
                # Format basic task info
                task_description = _format_task_basic_info(orphan, colorize_output, show_ids, show_score, show_tags, tag_style, show_type_emoji)
...
def _format_hierarchical_task(
    ...
    # Skip if task is already displayed
    task_id = task.get('id')
    ...
    # Convert any escaped newlines in task name
    task_name = _convert_escaped_newlines(task_name)
    
    # Build prefix based on level and branch map
    prefix = ""
    ...
    task_info = _format_task_basic_info(
        task,
        colorize_output=colorize_output,
        show_full=False,  # We'll handle detailed content separately
        show_score=show_score,
    ...
    # Add the task line
    lines = [f"{prefix}{connector}{task_info}"]
    ...
    if show_tags and task.get('tags'):
        ...
        # CRITICAL: Maintain pipe alignment for additional content by using the same prefix structure
        tag_prefix = prefix + ("  " if is_last_at_level else "│ ")
    ...
    if show_content_preview and task.get('description'):
        ...
        if description:
            # Convert escaped newline sequences to actual newlines
            description = _convert_escaped_newlines(description)
            
            # Get indentation prefix for description
            # CRITICAL: Maintain consistent pipe alignment for tree validation
            desc_prefix = prefix + ("  " if is_last_at_level else "│ ")
            
            # Handle multiline descriptions
            if '\n' in description:
                # Add description header with document emoji
                lines.append(f"{desc_prefix}📝 Description:")
                
                # Split into lines and process each line with proper indentation
                desc_lines = description.split('\n')
                desc_content_prefix = desc_prefix + "  "  # Additional indentation
                for line in desc_lines:
                    if line.strip():  # Skip empty lines
                        if len(line) > content_preview_length:
                            line = line[:content_preview_length-3] + '...'
                        lines.append(f"{desc_content_prefix}{line}")
            else:
                # Single line description
                if len(description) > content_preview_length:
                    description = description[:content_preview_length-3] + '...'
                # Add description with document emoji
                lines.append(f"{desc_prefix}📝 {description}")
    ...
    if show_relationships and task.get('relationships'):
        ...
        if relationships:
            # Get indentation prefix for relationships
            # CRITICAL: Maintain consistent pipe alignment for tree validation
            rel_prefix = prefix + ("  " if is_last_at_level else "│ ")
            
            # Process each relationship type
            for rel_type, related_tasks in relationships.items():
                # Skip if no tasks in this relationship
                if not related_tasks:
                    continue
                ...
                # Add relationship header with sync emoji
                rel_line = f"{rel_prefix}🔄 {rel_title}:"
                lines.append(rel_line)
                
                # Add each related task (up to 3 to prevent overly long output)
                for i, rel_task_id in enumerate(related_tasks[:3]):
                    # Try to get related task name
                    rel_task_name = "Unknown Task"
                    if rel_task_id in tasks_map:
                        rel_task = tasks_map[rel_task_id]
                        rel_task_name = rel_task.get('name', 'Unknown Task')
                        # Convert any escaped newlines in related task name
                        rel_task_name = _convert_escaped_newlines(rel_task_name)
                    
                    # Add related task with proper indentation
                    # CRITICAL: Use additional level of indentation while maintaining pipe alignment
                    task_prefix = rel_prefix + "  "
                    lines.append(f"{task_prefix}∟ {rel_task_name}")
                
                # If there are more than 3 related tasks, add a count
                if len(related_tasks) > 3:
                    task_prefix = rel_prefix + "  "
                    lines.append(f"{task_prefix}∟ ... and {len(related_tasks) - 3} more")
    ...
    if show_comments > 0 and task.get('comments'):
        ...
        if comments:
            ...
            # Get indentation prefix for comments
            # CRITICAL: Maintain consistent pipe alignment for tree validation
            comment_prefix = prefix + ("  " if is_last_at_level else "│ ")
            
            # Add comment header with speech bubble emoji
            lines.append(f"{comment_prefix}💬 Recent Comments:")
            ...
            for comment in recent_comments:
                ...
                if '\n' in comment_text:
                    ...
                    # Single line comment
                    if len(comment_text) > content_preview_length:
                        comment_text = comment_text[:content_preview_length-3] + '...'
                    lines.append(f"{sub_prefix}{comment_text}")
...
def _get_subtasks(task: Dict[str, Any], tasks_map: Dict[str, Dict], include_completed: bool = False) -> List[Dict]:
    ...
    for potential_subtask in tasks_map.values():
        if potential_subtask.get('parent_id') == task_id:
            if include_completed or str(potential_subtask.get('status', '')).lower() != 'complete':
...
def format_task_list(
    ...
    Parameters:
        ...
        show_descriptions: Whether to show task descriptions
        show_score: Whether to show task scores
        id_at_beginning: Whether to show IDs at beginning of line (vs end)
    ...
    logger.debug(f"format_task_list called with {len(tasks)} tasks, show_ids={show_ids}")
    ...
    # Track displayed task IDs for summary statistics
    displayed_task_ids = set()
    ...
    for task in sorted_tasks:
        ...
        task_effort = task.get('effort', 0)
        
        # Add debug log
        logger.debug(f"Formatting task: {task_id} - {task_name}, show_ids={show_ids}")
        
        # Truncate task name if requested
        if truncate and len(task_name) > truncate_length:
            task_name = task_name[:truncate_length-3] + '...'
        
        # Format with colors if requested
        if colorize:
            ...
            emoji = ""
            if show_type_emoji:
                emoji = TASK_TYPE_EMOJI_MAP.get(task_type, "📋") + " "
            
            # Format task status with color
            if task_status:
                status_text = colorize(f"[{task_status}]", status_color(task_status))
            else:
                status_text = ""
            ...
            # Always format task ID (but only include if show_ids is True)
            id_text = ""
            if show_ids and task_id:
                id_formatted = colorize(task_id, TextColor.BRIGHT_BLACK)
                if id_at_beginning:
                    id_text = f"[{id_formatted}] "
                else:
                    id_text = f" [{id_formatted}]"
            
            # Format score if requested
            score_text = ""
            if show_score:
                if task_effort > 0:
                    score_text = f" [E:{task_effort}]"
                if task_score > 0:
                    score_text += f" [S:{task_score}]"
            
            # Build the main task line - place ID at beginning or end based on id_at_beginning
            if id_at_beginning:
                task_line = f"{id_text}{emoji}{task_name} {status_text} {priority_text}{score_text}"
            else:
                task_line = f"{emoji}{task_name} {status_text} {priority_text}{id_text}{score_text}"
                
            # Format tags if requested
            if show_tags and task_tags:
                # Add tags on a new line with proper indentation and tag emoji
                tags_text = "    │ " + TAG_EMOJI + " " + format_tags(task_tags, colorize, tag_style)
                task_line = f"{task_line}\n{tags_text}"
                
            # Add description if requested
            if show_descriptions and task.get('description'):
                description = task.get('description', '')
                if len(description) > truncate_length:
                    description = description[:truncate_length-3] + '...'
            ...
            # Build the main task line with task ID if requested
            if show_ids and task_id:
                if id_at_beginning:
                    id_section = f"[{task_id}] "
                    task_line = f"{id_section}{emoji}{task_name} {status_text} {priority_text}{score_text}"
                else:
                    id_section = f" [{task_id}]"
                    task_line = f"{emoji}{task_name} {status_text} {priority_text}{id_section}{score_text}"
            else:
                task_line = f"{emoji}{task_name} {status_text} {priority_text}{score_text}"
                
            # Format tags if requested
            if show_tags and task_tags:
                # Add tags on a new line with proper indentation and tag emoji
                tags_text = "    │ " + TAG_EMOJI + " " + format_tags(task_tags, False, tag_style)
                task_line = f"{task_line}\n{tags_text}"
    ...
    total_displayable_tasks = len(sorted_tasks)
    completed_displayable_tasks = sum(1 for task in sorted_tasks if str(task.get('status', '')).lower() == 'complete')
    total_task_ids = len(displayed_task_ids)
    
    # Count duplicates by comparing displayed tasks vs unique IDs
    duplicate_count = 0
...
def _format_task_basic_info(
    ...
    if colorize_output:
        ...
        # Add ID if requested (in muted color)
        id_text = f"[{colorize(task_id, TextColor.BRIGHT_BLACK)}]" if show_ids and task_id else ""
...
def _format_subtask_hierarchy(
    ...
    for idx, subtask in enumerate(sorted_subtasks):
        ...
        task_description = _format_task_basic_info(subtask, colorize_output, show_ids, show_score, show_tags, tag_style, show_type_emoji)
        lines.append(f"{current_prefix}{branch}{task_description}")
        
        # Add description if requested
        if show_descriptions and subtask.get('description'):
            description = subtask.get('description', '')
            if len(description) > description_length:
                description = description[:description_length-3] + '...'
            desc_prefix = current_prefix + ("  " if is_last else "│ ")
            lines.append(f"{desc_prefix}📝 {description}")
            
        # Add relationships if requested
        if show_relationships and subtask.get('relationships'):
            relationships = subtask.get('relationships', {})
            for rel_type, related_tasks in relationships.items():
                if not related_tasks:
                    continue
                rel_prefix = current_prefix + ("  " if is_last else "│ ")
                rel_title = rel_type.replace('_', ' ').title()
                lines.append(f"{rel_prefix}🔄 {rel_title}: {', '.join(related_tasks[:3])}" +
                           (f" +{len(related_tasks) - 3} more" if len(related_tasks) > 3 else ""))
                           
        # Add comments if requested
        if show_comments > 0 and subtask.get('comments'):
            comments = subtask.get('comments', [])
            comment_count = min(show_comments, len(comments))
            if comment_count > 0:
                recent_comments = comments[-comment_count:]
                comment_prefix = current_prefix + ("  " if is_last else "│ ")
                lines.append(f"{comment_prefix}💬 Recent Comments:")
                for comment in recent_comments:
                    comment_text = comment.get('text', '')
                    comment_author = comment.get('author', 'Anonymous')
...
def fix_tree_structure(tree_lines: List[str]) -> List[str]:
    ...
    for i, line in enumerate(fixed_lines):
        ...
        for j, char in enumerate(line):
            ...
            elif char in ('├', '└') and j % 2 == 0:  # Only consider properly aligned branch chars
                if level not in pipe_positions:
                    pipe_positions[level] = set()
                pipe_positions[level].add(j)
            ...
            for pos in sorted(positions):
                ...
                if (line_chars[pos] == ' ' and 
                    ...
                    if (pos > 0 and pos < len(line_chars) - 1 and 
                        ...
                        (line_chars[pos+1] == ' ' or line_chars[pos+1] in ('├', '└'))):
                        line_chars[pos] = '│'
